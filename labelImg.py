...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def setDirty(self):
        ...
        self.keyFrames.add(frameNum)
        # self.interpolateFrames()
        # print(self.keyFrames)
        # print(self.findClosestKeyFrame())
    ...
    def ratioForCenter(self, centerx1, centery1, centerx2, centery2, timeDiff):
        ...
        yRatio = (centery2 - centery1) / timeDiff
        return (xRatio, yRatio)

    def ratioForWH(self, points1, points2, timeDiff):
        width1 = self.distanceFormula(points1[0][0], points1[0][1], points1[1][0], points1[1][1]) / 2
        height1 = self.distanceFormula(points1[1][0], points1[1][1], points1[2][0], points1[2][1]) / 2
        width2 = self.distanceFormula(points2[0][0], points2[0][1], points2[1][0], points2[1][1]) / 2
        height2 = self.distanceFormula(points2[1][0], points2[1][1], points2[2][0], points2[2][1]) / 2

        ratioW = (width2 - width1) / timeDiff
        ratioH = (height2 - height1) / timeDiff

        return (ratioW, ratioH)

    def ratioAngle(self, angle1, angle2, timeDiff):
    ...
    def rotatePoint(self, p, center, theta):
        order = p-center;
    ...
    def calcPoints(self, points, angle, ratioCenterx, ratioCentery, ratioW, ratioH, ratioAngle):
        ...
        centerx, centery = self.calcCenter(points)

        newAngle = angle + ratioAngle

        newcenterx = centerx + ratioCenterx
        newcentery = centery + ratioCentery

        newXleft = newcenterx - (width + ratioW)
        newXright = newcenterx + (width + ratioW)
        newYtop = newcentery + (height + ratioH)
        newYbot = newcentery - (height + ratioH)
        ...
        center = QPointF(newcenterx, newcentery)

        rotatedPoints = []
        for x, y in newPoints:
            qp = QPointF(x, y)
            result = self.rotatePoint(qp, center, newAngle)
            rotatedPoints.append((result.x(), result.y()))
...
# COME BACK HERE
    ...
    def interpolateFrames(self):
        ...
        if (closestKeyFrame != -1):
            ...
            for shape in self.canvas.shapes:
                ...
                currentPoints = self.convertPoints(shape.points)
                frame = str(closestKeyFrame).zfill(len(frameStr))
                cutNum = len(frame) + 4
                path = self.filePath[:-cutNum] + frame + ".xml"
                readKey = PascalVocReader(path)
                ...
                ratioCenterx, ratioCentery = self.ratioForCenter(centerx1, centery1, centerx2, centery2, timeDiff)
                ratioW, ratioH = self.ratioForWH(readKey.shapes[i][1], currentPoints, timeDiff)
            ...
            for i in range(closestKeyFrame + 1, currentFrameNum):
                ...
                for i, val in enumerate(read.shapes):
                    ...
                    read.shapes[i] = ((name, newPoints, newAngle, True, None, None, False))
    ...
    def findPrevXML(self):
        ...
        if (frameNum == 0):
            ...
            frameNum = frameNum - 1
            prevFrame = str(frameNum).zfill(len(frameStr))
    ...
    # Checks to see if the curent frame's xml file exists.
    def checkCurrentXML(self):
    ...
    # Gets the previous frame's xml file if it exists. Returns None otherwise.
    def createLabelFile(self):
        if self.filePath is None:
            return
        current = self.filePath[:-4] + ".xml"
        old = self.findPrevXML()
        if os.path.isfile(current):
            return LabelFile(current)
        if self.checkCurrentXML() is False and self.checkPrevXML() is not None:
            oldLabelFile = LabelFile(old)
            self.addNextXML(old, current, oldLabelFile)
            return LabelFile(current)
        else:
            return None
    ...
    def loadFile(self, filePath=None):
        ...
        if unicodeFilePath and os.path.exists(unicodeFilePath):
            ...
            self.labelFile = self.createLabelFile()
            # RIGHT HERE
            if self.labelFile:
